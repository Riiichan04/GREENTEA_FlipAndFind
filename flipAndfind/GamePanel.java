...
public class GamePanel extends JPanel {
    ...
    int count = 0;
    public GamePanel(int col, int row, int w, int h) {
        setBounds(0, 0, w, h);
        setLayout(null);
        status = 0;
        setCol(col);
        ...
        scoreLabel.setFont(new Font("Nunito", Font.PLAIN, 20));

        timeCount = new Timer(1000, checkTime());
        timeCount.start();

        timeLabel.setText("Thời gian: " + setTimeRemaining(30000*col*row));
    ...
    public void drawBTN(int col, int row) {
        ImageIcon[] picIcon = new ImageIcon[col * row];
    ...
    public void actionEqualValue() throws InterruptedException {
        ...
        if (countTrue == (col*row)/2) {
            timeCount.stop();
            FrameWinNotice win = new FrameWinNotice(score);
            win.setVisible(true);
    ...
    public ActionListener checkTime() {
        return e -> {
            count+=1000;
            timeLabel.setText("Thời gian: " + setTimeRemaining(30000*col*row - count));
            if (count == 30000*col*row) {
                timeCount.stop();
                timeCount.stop();
                FrameLose frameLose = new FrameLose(score, false);
                JFrame frame = (JFrame) SwingUtilities.getWindowAncestor(this);
                frame.dispose();
            }
        };
